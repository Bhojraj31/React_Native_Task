...
const getRangeSalary = (data: DataType) => {
    if (data?.data?.length > 0) {
        const d = data.data;

        const obj: { [key: number]: number[] } = {};
        d.forEach((item) => {
            const ageGroup = item.employee_age < 30 ? 0
                : item.employee_age < 40 ? 1
                    : item.employee_age < 50 ? 2
                        : 3;
            if (!obj[ageGroup]) {
                obj[ageGroup] = [];
            }
            obj[ageGroup].push(item.employee_salary);
        });
        ...
        const result = Object.values(obj).map((salaries) => {
            ...
            return { value: Math.round(average) };
        });
...
const categorizeSalaries = (salaryArrays: number[][]) => {
    ...
    keys.sort((a, b) => {
        let aStart = parseFloat(a.split('-')[0]);
        let bStart = parseFloat(b.split('-')[0]);
...
const EmployeeVsSalary = () => {
    const bar_chart_data = getRangeSalary(data);

    return (
        <View style={{ flex: 1 }}>
            <Text>EmployeeVsSalary</Text>
            <BarChart
                frontColor={'lightgreen'}
                barWidth={22}
                data={bar_chart_data.result}
                yAxisLabelWidth={100}
                height={500}
                width={300}
                spacing={50}
                yAxisLabelTexts={categorizeSalaries(bar_chart_data.ranges)}
            />
        </View>
    );
};

export default EmployeeVsSalary;
